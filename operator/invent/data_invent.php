...
$ss = mysqli_query($koneksi, "SELECT * FROM inventaris");
$as = mysqli_fetch_array($ss);
?>
<div class="col-12">
        ...
        <div class="card shadow">
          <div class="card-body">
            ...
            <table class="table datatables" id="dataTable-1">
              ...
              WHERE inventaris.kode_jurusan = $data_idjurusan");
                while ($data = $sql->fetch_assoc()) {
                ?>
                  <tr>
                    <td align="center">
                      <?php if ($data['stat'] == 0) { ?>
                        <a href="?page=berita-penerimaan&kode=<?= base64_encode($data['id_inventaris']) ?>" class="btn mb-2 btn-primary">Terima</a>
                      <?php }else{ ?>
                      <div class="custom-control custom-checkbox">
...
<h5><span class="foo bg-warning" style="color: black;"></span></h5>
</div> <!-- .col-12 -->
...
if (isset($_POST['sub'])) {
  ...
  foreach ($checkbox1 as $chk1) {
    ...
    if ($query_cek > 0) {
      echo "<script>window.alert('Barang Sudah di Entry ke Perawatan !')
		window.location='index.php?page=data-perawatan'</script>";
    } else {
      $in_ch = mysqli_query($koneksi, "INSERT INTO perawatan (id_perawatan, id_inventaris, tanggal_perawatan) values (NULL,'$chk1','$date')");
    }
  }
  if ($in_ch) {
    mysqli_query($koneksi, "INSERT INTO aktivitas_log (tanggal_log,ip,agent,browser,status,id_level)VALUES('$currentDateTime','$ip','$agent','$browser','$perawatan','$data_level')");
    echo "<script>alert('Barang Perawatan Berhasil Ditambahkan')  
      window.location='index.php?page=data-perawatan'</script>";
  } else {
    echo '<script>alert("Maaf Terjadi Kesalahan !");
      var fe = document.querySelector(".foo");
      fe.textContent = "NB : Silahkan Terima dahulu sebelum melakukan entry ke perawatan ,Jika sudah Untuk Menambahkan Data Barang ke Perawatan, Silahkan Checklist Terlebih Dahulu Pada Kolom Tabel Sebelah Kiri (Rawat)";
      </script>';
  }
}
?>